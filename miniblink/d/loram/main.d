...
void test(ref SSD1306 oled)
{
  ...
  void drawImageBuffer(immutable ubyte[1025] buf)
  {
    oled.clear();
    oled.drawImgBuf(buf);
    oled.refresh();
  ...
  static immutable ubyte[1025] buf1 = imgToBuffer(img1);
  static immutable ubyte[1025] buf2 = imgToBuffer(img2);
  foreach (_; 0 .. 10)
  {
    drawImageBuffer(buf1);
    sleepMs(100);
    drawImageBuffer(buf2);
    sleepMs(100);
  }
  oled.turnOff();
}
...
struct Gfx
{
  ...
  void drawImgBuf(immutable ubyte[1025] buf)
  {
    this.buffer[] = buf[];
  }
}
...
ubyte[1025] imgToBuffer(immutable ubyte[1024] img)
{
  ...
  foreach(y; 0 .. 64)
  {
    auto yo = (y / 8) * 128;
    auto ybit =  1 << (y & 7);
    foreach(x; iota(0, 128, 8))
      foreach(xi; 0 .. 8)
        if ((img[yo + (x / 8)] & (128 >> xi)) > 0)
          buffer[1 + yo + x + xi ] |=  ybit;
  }
  return buffer;
}
